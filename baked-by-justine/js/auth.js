...
<div id="authOverlay" role="dialog" aria-modal="true">
  <div id="authModal">
    <div class="am-header">
      <button class="am-close" id="amClose" aria-label="Close">
      ...
      <div class="am-logo">
        ...
        Baked By <span>Justine</span>
      </div>
      <div class="am-tabs">
        <button class="am-tab active" id="amTabLogin"    onclick="switchTab('login')">Sign In</button>
        <button class="am-tab"        id="amTabRegister" onclick="switchTab('register')">Create Account</button>
      </div>
    </div>
    <div class="am-body">
      ...
      <div class="am-panel active" id="amPanelLogin">
        <h2 class="am-title">Welcome <em>Back</em></h2>
        <p class="am-sub">Sign in to order faster and track your history.</p>
        <div class="am-feedback" id="fbLogin"></div>
        <div class="am-form">
          <div class="am-group">
            <label for="liEmail">Email Address</label>
            <div class="am-field-wrap">
              <input type="email" id="liEmail" placeholder="you@email.com" autocomplete="email" />
            </div>
            <span class="am-field-err" id="liEmailErr">Please enter a valid email.</span>
          </div>
          <div class="am-group">
            <label for="liPw">Password</label>
            <div class="am-field-wrap">
              <input type="password" id="liPw" placeholder="••••••••" autocomplete="current-password" />
              <button class="am-pw-toggle" type="button" onclick="togglePw('liPw',this)" aria-label="Toggle">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
              </button>
            </div>
            <span class="am-field-err" id="liPwErr">Please enter your password.</span>
          </div>
          <button class="am-submit" id="btnLogin" onclick="doLogin()">
        ...
        <div class="am-switch">No account? <button onclick="switchTab('register')">Create one free</button></div>
        <div class="am-guest"><button onclick="closeAuthModal()">Continue as guest →</button></div>
      </div>

      <!-- REGISTER -->
      <div class="am-panel" id="amPanelRegister">
        <h2 class="am-title">Join the <em>Bakery</em></h2>
        <p class="am-sub">Free account — faster checkout, order history.</p>
        <div class="am-feedback" id="fbRegister"></div>
        <div class="am-form">
          <div class="am-row">
            <div class="am-group">
              <label for="rgFirst">First Name</label>
              <div class="am-field-wrap">
                <input type="text" id="rgFirst" placeholder="Sarah" autocomplete="given-name" />
              </div>
              <span class="am-field-err" id="rgFirstErr">Required.</span>
            </div>
            <div class="am-group">
              <label for="rgLast">Last Name</label>
              <div class="am-field-wrap">
                <input type="text" id="rgLast" placeholder="Mitchell" autocomplete="family-name" />
              </div>
              <span class="am-field-err" id="rgLastErr">Required.</span>
            </div>
          </div>
          <div class="am-group">
            <label for="rgEmail">Email Address</label>
            <div class="am-field-wrap">
              <input type="email" id="rgEmail" placeholder="you@email.com" autocomplete="email" />
            </div>
            <span class="am-field-err" id="rgEmailErr">Please enter a valid email.</span>
          </div>
          <div class="am-group">
            <label for="rgPw">Password</label>
            <div class="am-field-wrap">
              <input type="password" id="rgPw" placeholder="Min. 8 characters" autocomplete="new-password" oninput="pwStrength(this.value)" />
              <button class="am-pw-toggle" type="button" onclick="togglePw('rgPw',this)" aria-label="Toggle">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
              </button>
            </div>
            <div class="am-pw-bars">
              <div class="am-pw-bar" id="pb1"></div><div class="am-pw-bar" id="pb2"></div>
              <div class="am-pw-bar" id="pb3"></div><div class="am-pw-bar" id="pb4"></div>
            </div>
            <div class="am-pw-hint" id="pwHint"></div>
            <span class="am-field-err" id="rgPwErr">Minimum 8 characters.</span>
          </div>
          <button class="am-submit" id="btnRegister" onclick="doRegister()">
        ...
        <div class="am-switch">Already have one? <button onclick="switchTab('login')">Sign in</button></div>
        <div class="am-guest"><button onclick="closeAuthModal()">Continue as guest →</button></div>
      </div>
...
<div id="authToast"></div>`);
  ...
  document.querySelectorAll('#authModal input').forEach(inp => {
    inp.addEventListener('keydown', e => {
      ...
      document.getElementById('amPanelLogin').classList.contains('active') ? doLogin() : doRegister();
    });
    inp.addEventListener('input', function() {
      this.classList.remove('err');
      const el = document.getElementById(this.id + 'Err');
