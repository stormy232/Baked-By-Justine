/* ============================================================
   BAKED BY JUSTINE — cart.js  v2
   - Logged in  → syncs with database via cart_db.php
   - Guest      → uses localStorage
   Cart persists across devices when logged in.
   ============================================================ */

const cart = {
  items: {},       // product_id -> item object
  isLoggedIn: false,

  // ── Boot: check session then load cart ──────────────────
  async init() {
    try {
      const res  = await fetch('api/auth.php?action=session');
      const data = await res.json();
      this.isLoggedIn = !!(data.logged_in && data.user);
    } catch(e) {
